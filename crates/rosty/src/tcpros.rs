...
use std::io;
use std::io::Cursor;
pub use subscriber::{PublisherConnectError, Subscriber};
...
async fn read_packet<U: AsyncRead + Unpin>(stream: &mut U) -> Result<Vec<u8>, io::Error> {
    ...
    std::io::Cursor::new(&mut out).write_u32::<LittleEndian>(length)?;
...
async fn read_packet_size<U: AsyncRead + Unpin>(stream: &mut U) -> Result<u32, io::Error> {
    ...
    Ok(byteorder::ReadBytesExt::read_u32::<LittleEndian>(
        &mut Cursor::new(&data),
    )?)
}
